...
namespace Ars.Common.OpcUaTool.Node.Device
{
    ...
    public class NodeMelsecMc : DeviceNode, IXmlConvert
    {
        ...
        public override void LoadByXmlElement( XElement element )
        {
            ...
            NetworkNumber = byte.Parse( element.Attribute( "NetworkNumber" )?.Value ?? "0" );
            NetworkStationNumber = byte.Parse( element.Attribute( "NetworkStationNumber" )?.Value ?? "0" );
            IsBinary = bool.Parse( element.Attribute( "IsBinary" )?.Value ?? "true");
        }
